...
BEGIN
    ...
    IF num_users = 0 THEN
        CREATE ROLE ledgersmb WITH SUPERUSER LOGIN NOINHERIT ENCRYPTED PASSWORD '$LSMBDBPW';
    ELSE
        ALTER ROLE ledgersmb WITH SUPERUSER LOGIN NOINHERIT ENCRYPTED PASSWORD '$LSMBDBPW';
    END IF;
END
\$\$
;
